import { sha256FromString, sha256 } from "ethereumjs-util";
...
export const namehash = (name: string) => {
  ...
  if (name !== "") {
    ...
    for (let i = labels.length - 1; i >= 0; i--) {
      const labelHash = sha256FromString(labels[i]).toString("hex");
      node = sha256(Buffer.from(node + labelHash, "hex")).toString("hex");
    }
  }
...
export const nodehash = (name: string) => {
  ...
  if (label) {
    return "0x" + sha256FromString(label).toString("hex");
  } else {
    return "";
  }
};
